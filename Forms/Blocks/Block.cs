...
{
    public class Block : IEnumerable<int>, IComparable
    {
        ...
        public Block(List<int> members, int n)
        {
        ...
        {
            Block c = obj as Block;
            if (c == null)
                throw new BlockException("Cannot compare to null Block or non-Block object.");
            ...
            for (int i = 0; i < _memberSet.Length; ++i)
                if (_memberSet[i] - c._memberSet[i] != 0)
                    return -1 * (_memberSet[i] - c._memberSet[i]);
        ...
        public void ComputeCohesion(Matrix SESE)
        {
            ...
            /*
            _cohesion = 0.0;
            List<double> cohesionArray = new List<double>();
            for (int i = 0; i < Count; i++)
            {
                for (int j = i + 1; j < Count; j++)
                {
                    if (IntContains(j) == 1)
                        cohesionArray.Add(SESE[IntContains(i), IntContains(j)]);
                }
            }
            ...
            else
            {
                ...
                _cohesion = IntContains(3);// / cohesionArray.Count;
